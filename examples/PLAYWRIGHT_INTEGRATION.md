...
class TestEventClient {
  constructor(serverAddress = 'localhost:50051') {
    const PROTO_PATH = path.join(__dirname, '../proto/testsystem/v1/observer/observer.proto');
...
const { generateId, timestampFromDate, durationFromMs, mapStatus } = require('./mappers');
...
| project.name | TestSuiteRun.project_name | Browser/device config |
